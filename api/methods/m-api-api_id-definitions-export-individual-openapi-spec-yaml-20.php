<?php
$route = '/api/:api_id/definitions/export/individual/openapi/spec/2.0/';
$app->get($route, function ($api_id)  use ($app,$awsAccessKey,$awsSecretKey,$awsSiteBucket,$awsRootURL,$guser,$gpass){
	...
	if($DefinitionResult && mysql_num_rows($DefinitionResult))
		...
		$info_description = str_replace(chr(13),"",$info_description);
		$info_description = str_replace(chr(10),"",$info_description);
		$info_description = htmlentities($info_description);
		...
		//$SwaggerPath = json_encode($SwaggerPath);
