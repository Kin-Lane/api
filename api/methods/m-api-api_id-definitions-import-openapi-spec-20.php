<?php
$route = '/api/:api_id/definitions/import/openapi-spec/2.0/';
$app->post($route, function ($api_id)  use ($app){
	...
	$SwaggerVersion = "2.0";
	if(isset($ObjectResult['swagger']))
		{
		$SwaggerVersion = $ObjectResult['swagger'];
		}

	$SwaggerBasePath = "/";
	if(isset($ObjectResult['basePath']))
		{
		$SwaggerBasePath = $ObjectResult['basePath'];
		}

	$SwaggerResourcePath = "";
	if(isset($ObjectResult['host']))
		{
		$SwaggerResourcePath = $ObjectResult['host'];
		}

	$SwaggerInfo = "";
	if(isset($ObjectResult['info']))
		{
	...
	//echo "Contact URL: " . $SwaggerInfo_Contact_URL . "<br />";
	//echo "api_definition_id: " . $param['api_definition_id'] . "<br />";
	if(isset($param['api_definition_id']))
		{
		$api_definition_id = $param['api_definition_id'];	
		
		$SwaggerPathQuery = "SELECT * FROM api_swagger WHERE API_Swagger_ID = " . $api_definition_id;
		...
		if($SwaggerPathResults && mysql_num_rows($SwaggerPathResults))
			...
			$SwaggerPathQuery = "UPDATE api_swagger SET";
			$SwaggerPathQuery .= " import_date = '" . $Today . "'";
			$SwaggerPathQuery .= ", swagger = '2.0'";
			$SwaggerPathQuery .= ", info_title = '" . mysql_real_escape_string($SwaggerInfo_Title) . "'";
			$SwaggerPathQuery .= ", info_description = '" . mysql_real_escape_string($SwaggerInfo_Description) . "'";
			$SwaggerPathQuery .= ", info_termsOfService = '" . mysql_real_escape_string($SwaggerInfo_TermsOfService) . "'";
			
			$SwaggerPathQuery .= ", info_contact_name = '" . mysql_real_escape_string($SwaggerInfo_Contact_Name) . "'";
			$SwaggerPathQuery .= ", info_contact_url = '" . mysql_real_escape_string($SwaggerInfo_Contact_URL) . "'";
			$SwaggerPathQuery .= ", info_contact_email = '" . mysql_real_escape_string($SwaggerInfo_Contact_Email) . "'";
			
			$SwaggerPathQuery .= ", host = '" . mysql_real_escape_string($SwaggerResourcePath) . "'";
			$SwaggerPathQuery .= ", basePath = '" . mysql_real_escape_string($SwaggerBasePath) . "'";			
			
			$SwaggerPathQuery .= " WHERE API_Swagger_ID = " . $api_definition_id;
