<?php
$route = '/api/:api_id/definitions/utility/generate/tags/from/path/';
	...
	if($DefinitionResult && mysql_num_rows($DefinitionResult))
		...
		if($SwaggerPathResults && mysql_num_rows($SwaggerPathResults))
			...
			while ($SwaggerPathRow = mysql_fetch_assoc($SwaggerPathResults))
				...
				$tag = ucfirst(strtolower($pathArray[0]));							
					
				$P['from'] = $path;
				$P['to'] = $tag;
				array_push($ReturnObject, $P);
				
				$UpdateQuery = "UPDATE api_swagger_paths SET tags = '" . $tag . "' WHERE ID = " . $Swagger_Path_ID;
